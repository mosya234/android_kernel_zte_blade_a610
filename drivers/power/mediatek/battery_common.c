...
#ifdef CONFIG_WIND_MULTI_BAT_PROFILE_SUPPORT
...
//#ifdef CONFIG_TOUCHSCREEN_MTK_MSG5846
...
static void mt_battery_notify_VBatTemp_check(void)
{
	...
	//modify@zte.com.cn 20160304 begin
	...
	//zhanyoufei@wiind-mobi.com 20160304 end
...
#endif
//modify@zte.com.cn 20160127 end
...
// modify@zte.com.cn 20160530 begin			
//#ifdef CONFIG_TOUCHSCREEN_MTK_MSG5846
//                    DrvFwCtrlChargerDetection(0); // liupeng 20160530 charger plug-in
//#endif 
// modify@zte.com.cn 20160530 end
#if defined(CONFIG_MTK_DUAL_INPUT_CHARGER_SUPPORT)
			battery_log(BAT_LOG_CRTI,
...
//modify@zte.com.cn 20160609 begin
	...
	#ifdef CONFIG_WIND_E188F_MULTI_BAT_PROFILE_SUPPORT
	if(g_fg_battery_id==0){
	np = of_find_compatible_node(NULL, NULL, "mediatek,battery1");
	if (!np) {
		...
		if(g_fg_battery_id == 2)
			{
			np = of_find_compatible_node(NULL, NULL, "mediatek,battery2");
			if (!np) {
			/* printk(KERN_ERR "(E) Failed to find device-tree node: %s\n", path); */
			battery_log(BAT_LOG_CRTI, "Failed to find device-tree node: bat_comm\n");
			return -ENODEV;
