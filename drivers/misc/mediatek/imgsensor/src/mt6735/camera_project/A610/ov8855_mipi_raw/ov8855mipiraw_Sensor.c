...
//#define OV8855R1A_OTP_SUPPORT
#ifdef OV8855R1A_OTP_SUPPORT
struct otp_struct {
...
{
	int otp_flag, addr, temp, i, checksum2;
...
{
	int RG_Ratio_Typical = 0x121, BG_Ratio_Typical = 0x11B;
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id) 
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		spin_unlock(&imgsensor_drv_lock);
		do {
			if(read_cmos_sensor(0x300B)==0) break;
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	spin_unlock(&imgsensor_drv_lock);
	preview_setting();
	set_mirror_flip(imgsensor.mirror);//modify@zte.com.cn 20141231
	mdelay(10);
	return ERROR_NONE;
}	/*	preview   */
