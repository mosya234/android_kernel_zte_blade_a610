...
#include "s5k4h8mipi_Sensor_avc.h"
...
#define tRG_Ratio_typical 0x17F
#define tBG_Ratio_typical 0x12F
#define GbGr_ratio_Typical 0x203
//modify@zte.com.cn 20160322 end
#endif
static DEFINE_SPINLOCK(imgsensor_drv_lock);
static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = S5K4H8_AVC_SENSOR_ID,
...
static int read_otp_s5k4h8(struct S5K4H8_otp_struct *otp_ptr)
{
	...
	//modify@zte.com.cn 20160711 begin
    LOG_INF("wangkangmin avc s5k4h8 read flag=%x\n",otp_flag);
    if((otp_flag & 0xc0) == 0x40) 
	{
	  addr = 0x0A07; // group 1
	  LOG_INF("wangkangmin s5k4h8 group1\n");
	  otp_ptr->MID=read_cmos_sensor_8(0x0A05);//vendor id
	  LOG_INF("wangkangmin avc s5k4h8 vendor id=%x\n",otp_ptr->MID);
	}
	else if((otp_flag & 0x30) == 0x10) 
	{
	  addr = 0x0A0D; // group 2
	  LOG_INF("wangkangmin s5k4h8 group2\n");
	  otp_ptr->MID=read_cmos_sensor_8(0x0A0B);//vendor id
	  LOG_INF("wangkangmin avc s5k4h8 vendor id=%x\n",otp_ptr->MID);
	}
	//modify@zte.com.cn 20160711 end
	if(addr != 0)
	{
	 otp_ptr->flag=0x01;
	 ...
	 //modify@zte.com.cn 20160322 begin
     otp_ptr->RGr_ratio = (read_cmos_sensor_8(addr)<<2)+((read_cmos_sensor_8(addr+3)&0xc0)>>6);
     otp_ptr->BGr_ratio = (read_cmos_sensor_8(addr+1)<<2)+((read_cmos_sensor_8(addr+3)&0x30)>>4);
     otp_ptr->GbGr_ratio = (read_cmos_sensor_8(addr+2)<<2)+((read_cmos_sensor_8(addr+3)&0x0c)>>2);
	 //modify@zte.com.cn 20160322 end
...
static void preview_setting(void)
{
  ...
  write_cmos_sensor(0x0B00, 0x0180);// modify@zte.com.cn 20151120 LSC enable
  write_cmos_sensor(0x6028, 0x2000);
...
static void normal_capture_setting(void)
{
  ...
  write_cmos_sensor(0x030E, 0x00AF);
  write_cmos_sensor(0x3008, 0x0000);
  //add by modify@zte.com.cn for camera_OTP 20160509 begin
  write_cmos_sensor(0x3058, 0x0100);
  //add by modify@zte.com.cn for camera 20160509 end
  write_cmos_sensor(0x0100, 0x0100);	
	check_output_stream_on();
}
        ...
        do {
			...
			*sensor_id = read_cmos_sensor(0x6F12)+1;
            ...
            if (*sensor_id == imgsensor_info.sensor_id) {               
				...
				read_otp_s5k4h8(otp_ptr1);
                LOG_INF("liukun s5k4h8 avc read otp vendor id=%x\n",(*otp_ptr1).MID);
				 if((*otp_ptr1).MID == 0x34)
			 	 {
                   kfree(otp_ptr1);
...
UINT32 S5K4H8_MIPI_RAW_AVC_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
	/* To Do : Check Sensor status here */
	if (pfFunc!=NULL)
		*pfFunc=&sensor_func;
	return ERROR_NONE;
}	/*	S5K4H8_MIPI_RAW_AVC_SensorInit	*/
/*
