...
#define tRG_Ratio_typical 0x125
#define tBG_Ratio_typical 0x143
...
static int read_otp_s5k4h8(struct S5K4H8_otp_struct *otp_ptr)
{
    ...
    LOG_INF("wangkangmin sijichun s5k4h8 read flag=%x\n",otp_flag);
    if((otp_flag & 0xc0) == 0x40) 
	{
	  addr = 0x0A07; // group 1
	  LOG_INF("wangkangmin s5k4h8 group1\n");
	  otp_ptr->MID=read_cmos_sensor_8(0x0A05);//vendor id
	  LOG_INF("wangkangmin sijichun s5k4h8 vendor id=%x\n",otp_ptr->MID);
	}
        ...
        do {
			...
			*sensor_id = read_cmos_sensor(0x6F12);
            ...
            if (*sensor_id == imgsensor_info.sensor_id) {
				...
				read_otp_s5k4h8(otp_ptr1);
                LOG_INF("liukun s5k4h8 sijichun read otp vendor id=%x\n",(*otp_ptr1).MID);
				 if((*otp_ptr1).MID == 0x10)
			 	 {
                   kfree(otp_ptr1);
