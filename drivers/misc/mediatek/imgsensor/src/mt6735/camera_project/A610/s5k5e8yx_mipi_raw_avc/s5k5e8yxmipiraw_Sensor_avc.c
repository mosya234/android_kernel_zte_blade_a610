...
#include "s5k5e8yxmipiraw_Sensor_avc.h"
...
static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = S5K5E8YX_AVC_SENSOR_ID,
...
static void preview_setting(void)
{
	...
	write_cmos_sensor(0x3400,0x01); //shade_bypass LSC off modify@zte.com.cn 20160617
	write_cmos_sensor(0x323B,0x02); //modify@zte.com.cn 20151124 mirror&flip cause lSC shading problem
	//Streaming on
...
static int read_otp_s5k5e8(struct S5K5E8_otp_struct *otp_ptr)
{
	...
	{
	  addr = 0x0A16; // group 2
	  otp_ptr->MID = read_cmos_sensor(0x0A0A);
	  vendor_id = otp_ptr->MID;
		...
		do {
			*sensor_id = return_sensor_id() + 1;
			if (*sensor_id == imgsensor_info.sensor_id) {	
                ...
                LOG_INF("wangkangmin S5K5E8 avc read otp vendor id=%x\n",(*otp_ptr1).MID);
...
UINT32 S5K5E8YX_MIPI_RAW_AVC_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
	/* To Do : Check Sensor status here */
	if (pfFunc!=NULL)
		*pfFunc=&sensor_func;
	return ERROR_NONE;
}	/*	S5K5E8YX_MIPI_RAW_AVC_SensorInit	*/
/*
